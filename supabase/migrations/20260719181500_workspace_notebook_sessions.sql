-- NotebookLM-style workspace notebook:
-- product_id null represents an "All sources" conversation/query that can read
-- every approved source document in the user's organization. Product notebooks
-- remain product-scoped plus org-wide documents.
...
drop policy if exists "users create own product sessions" on public.notebook_sessions;
drop policy if exists "users update own sessions" on public.notebook_sessions;

create policy "users create own notebook sessions"
...
set search_path = ''
as $$
  with notebook_scope as (
    select
      (select profile.org_id from public.profiles as profile where profile.id = (select auth.uid())) as org_id,
  ...
  paragraphs as (
    ...
    join notebook_scope on notebook_scope.org_id = document.org_id
    ...
    where notebook_scope.product_id is null
       or document.product_id = notebook_scope.product_id
