...
export async function extractDocumentTextCore(file: File): Promise<string | null> {
  const ext = documentExtension(file.name);
  if (PLAIN_TEXT_EXTENSIONS.has(ext)) return (await file.text()).trim();
  if (!PARSER_EXTENSIONS.has(ext)) return null;

  const { OfficeParser } = await import("officeparser");
  const bytes = new Uint8Array(await file.arrayBuffer());
  const fileType = ext === "markdown" ? "md" : ext === "htm" ? "html" : ext;
