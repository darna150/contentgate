...
import {
  coercePublishedTemplateFieldsToFit,
  formatPublishedTemplateFitIssues,
  publishedTemplateFieldFitIssues,
  publishedTemplateFitInstructions,
} from "@/lib/published-template-fit";
...
export async function POST(req: Request) {
  ...
  if (
    (!productTemplateId && !platformAssignmentId) ||
    (productTemplateId && platformAssignmentId)
  ) {
  ...
  if (
    getTemplateLayoutContract(tpl.layout_key)?.renderer === "published-design" &&
    !outputSizeKey
  ) {
    return Response.json(
      { error: "Choose an output size before generating this template." },
  ...
  const typographyInstructions = outputSizeKey
    ? publishedTemplateFitInstructions({
        layoutKey: tpl.layout_key,
        sizeKey: outputSizeKey,
        definition: tpl.template_definition,
      })
    : [];
  ...
  for (let attempt = 0; attempt < LEGACY_GENERATION_ATTEMPTS; attempt += 1) {
    ...
    const geometryIssues = outputSizeKey
      ? await publishedTemplateFieldFitIssues({
          layoutKey: tpl.layout_key,
          sizeKey: outputSizeKey,
          fields: structured,
          definition: tpl.template_definition,
    ...
    retryReasons = [
      ...editableFields.flatMap((key) =>
        (configuredIssues[key] ?? []).map(
          (issue) => `${key}: ${issue.message}`
        )
      ),
      ...formatPublishedTemplateFitIssues(geometryIssues),
    ];
  ...
  if (!out && Object.values(structured).some(Boolean) && outputSizeKey) {
    structured = await coercePublishedTemplateFieldsToFit({
      layoutKey: tpl.layout_key,
      sizeKey: outputSizeKey,
      fields: structured,
      definition: tpl.template_definition,
  ...
  if (!out) {
    console.error("generated copy failed template fit validation:", {
      productTemplateId,
      outputSize: outputSizeKey,
      reasons: retryReasons,
    });
