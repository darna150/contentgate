...
| Availability | Health, worker liveness, queue recovery, provider retry, and failure alerts have owners | Five-minute authenticated cron, health/worker checks, structured logs, and bounded authenticated incident webhook are implemented | In progress — production route, named owner, and synthetic delivery proof required |
