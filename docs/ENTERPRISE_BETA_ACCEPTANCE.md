...
| Capacity | Concurrent login, upload, Ask, generation, review, and export meet an approved beta envelope | A bounded staging-only gate and proposed design-partner envelope now cover concurrent health, sign-in, and core reads; functional Ask latency/cost evidence also exists | In progress — run the exact-Preview read gate and implement the bounded stateful rows |
